...
struct GitHubRelease: Decodable {
    ...
    /// Extract version from release title (format: "iCanHazRepose x.x.x" or "iCHR x.x.x")
    var parsedVersion: String? {
        guard let name else { return nil }
        let pattern = #"(?:iCanHazRepose|iCHR)\s+v?(\d+\.\d+\.\d+)"#
...
class UpdateManager {
    ...
    private let repo = "iCanHazRepose"
    ...
    /// The first check fires after `checkInterval` (24h), so we skip the first 24 hours.
    ...
    func checkForUpdates(manual: Bool, completion: @escaping (Result<UpdateInfo?, Error>) -> Void) -> Bool {
        ...
        Task {
            do {
                ...
                // Find the latest non-prerelease release that has a valid version and iCHR.zip
                let latestRelease = releases
                    .filter { $0.parsedVersion != nil && !$0.prerelease }
                    .sorted { SemVer($0.parsedVersion!) > SemVer($1.parsedVersion!) }
                    .first { release in
                        release.assets.contains(where: { $0.name == "iCHR.zip" })
                    }
                ...
                if !manual && skippedVersion == version {
                ...
                let asset = release.assets.first(where: { $0.name == "iCHR.zip" })!
    ...
    func downloadAndInstall(_ updateInfo: UpdateInfo) async throws {
        ...
        // Download zip
        let downloadURL = tmpdir.appendingPathComponent("iCHR.zip")
    ...
    var skippedVersion: String? {
        get { UserDefaults.standard.string(forKey: SettingsKey.skippedUpdate) }
        set { UserDefaults.standard.set(newValue, forKey: SettingsKey.skippedUpdate) }
    }
